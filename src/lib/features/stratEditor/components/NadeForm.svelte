<script lang="ts">
  ...
  const removeNade = (id: number | undefined) => {
    if (id === undefined) return;

    nades = nades.filter((nade) => nade.id !== id);
    activeNade = undefined;
  };
</script>
...
{:else}
  <div class="grid gap-4 content-start">
    ...
    <div class="flex flex-wrap items-center justify-between gap-2">
      ...
      />
      <Button on:click={() => removeNade(activeNade?.id)} variant="destructive"
        >Remove nade</Button
      >
    </div>
  </div>
