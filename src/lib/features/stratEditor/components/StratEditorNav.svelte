...
<ol class="flex mb-5">
  <li class="flex mr-4 items-end">
    <Button on:click={navPrev} disabled={activeStep === FormSteps.INFO}
      ><svg
        class="fill-white"
  ...
  <li
    class="grid flex-grow text-center border-primary {activeStep ===
    FormSteps.INFO
      ? 'border-b-4 text-primary'
      : ''}"
  ...
  <li class="flex ml-4 items-end">
    <Button on:click={navNext} disabled={activeStep === FormSteps.OVERVIEW}
      ><svg
        class="fill-white"
        xmlns="http://www.w3.org/2000/svg"
        height="1em"
