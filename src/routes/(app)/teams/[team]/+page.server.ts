...
export const actions = {
  invitePlayer: async ({ request, locals }) => {
    ...
    const { username, teamId, role, senderId } = form.data;
    ...
    // Make sure user is not already on the team
    const { data: teamMember } = await locals.supabase
      .from('team_members')
      .select('*')
      .eq('team_id', teamId)
    ...
    if (teamMember && teamMember.length > 0) {
      return fail(400, {
        form,
        message: `${username} is already on this team!`,
      });
    }

    // Make sure user is not already invited
    const { data, error: someError } = await locals.supabase
      .from('notifications')
      .select('*, type:type ( name )')
      .eq('type.name', 'TEAM_INVITE')
      .eq('reference_id', teamId)
      .eq('recipient_id', userData[0].id)
      .eq('status', 'PENDING');
    ...
    // Fetch invitation type
    const { data: inviteData } = await locals.supabase
    ...
    // Invite user
    const { error } = await locals.supabase.from('notifications').upsert({
      type: inviteData.id,
      recipient_id: userData[0].id,
      sender_id: Number(senderId),
      reference_id: Number(teamId),
      details: {
        teamId: teamData.id,
